process SPOQC_ANALYSIS_CLUSTER {
    ...
    if (workflow.profile.tokenize(',').intersect(['conda', 'mamba']).size() >= 1) {
        error("SPOQC_ANALYSIS_CLUSTER module does not support Conda. Please use Docker / Singularity / Podman instead.")
    }
    ...
    """
    mkdir -p report/analysis/cluster
    touch report/analysis/rna_cluster.h5ad
    """
}
